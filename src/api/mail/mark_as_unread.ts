 ...
 * @param {Request} req - Request object, body is {@link MarkAsUnreadParams}
 * @returns Confirmation with message count
 */
export default async function main(req: Request) {
  const params = (await req.json()) as MarkAsUnreadParams;
  ...
  const statements = ids
    .map((id) => `set read status of (first message of targetBox whose id is ${id}) to false`)
    .join("\n        ");
  ...
  return Response.json({ result: `Marked ${ids.length} message(s) as unread` });
}
